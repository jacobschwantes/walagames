import { signOut, authOptions } from "@/auth";
...
const games = [
  {
    ...
    classNames: {
      ...
      card: "bg-sky-400 border-sky-500",
  ...
  },
  {
    ...
    classNames: {
      ...
      card: "bg-red-400 border-red-500",
...
const JoinLobbyOrHost = () => {
  return (
    <ul className="grid grid-cols-2 w-full gap-5">
      {games.map((game) => (
        <button key={game.name} className="group relative w-full aspect-video ">
          {/* bg border */}
          <div
            className={cn(
...
//       <div
//         className={cn(
//           " shadow-lg w-full h-full -translate-y-[6px] group-hover:-translate-y-[4px]  group-hover:brightness-105   duration-300    overflow-hidden rounded-2xl absolute border-t-[1px] border-x-[1px]  transition-all z-10 inset-0 ",
//           game.classNames.card
//         )}
//       >
//         {/* bg, border, shadow */}
...
//           )}
//         ></div>
//         <div className="absolute group-hover:-rotate-[68deg]  duration-300 transition-all  w-full -top-1/2 left-1/3 scale-95 -rotate-[75deg] text-white opacity-10 z-10 ">
//           <game.icon />
//         </div>
//       </div>
//       {/* Border and bg gradient */}
